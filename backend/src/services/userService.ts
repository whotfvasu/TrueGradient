...
export class UserService {
  static async getUserStats(userId: string): Promise<UserStats> {
    const [user, conversationCount, totalMessages, tokensUsed] =
      await Promise.all([
    ...
    const averageTokensPerMessage =
      totalMessages > 0 ? Math.round(totalTokensUsed / totalMessages) : 0;
  ...
  static async getUserCredits(userId: string): Promise<UserCreditsInfo> {
  ...
  static async addCredits(
    ...
    console.log(
      `Credits added: ${amount} to user ${updatedUser.username}${
        reason ? ` (${reason})` : ""
      }`
    );
  ...
  static async deductCredits(
    ...
    if (currentUser.credits < amount) {
      throw new Error(
        `Insufficient credits. Required: ${amount}, Available: ${currentUser.credits}`
      );
    }
    ...
    console.log(
      `Credits deducted: ${amount} from user ${updatedUser.username}${
        reason ? ` (${reason})` : ""
      }`
    );
  ...
  static async updateUserProfile(userId: string, data: { email?: string }) {
    const user = await prisma.user.findUnique({
      where: { id: userId },
    });
  ...
  static async bulkAddCredits(
    ...
    console.log(
      `Bulk credits added: ${amount} to ${result.count} users${
        reason ? ` (${reason})` : ""
      }`
    );
